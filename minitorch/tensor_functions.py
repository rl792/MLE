...
class Mul(Function):
    ...
    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, Tensor]:
        a2, b2 = ctx.saved_values
        return (
            grad_output.f.mul_zip(b2, grad_output),
            grad_output.f.mul_zip(a2, grad_output),
        )
...
class Sigmoid(Function):
    @staticmethod
    def forward(ctx: Context, t1: Tensor) -> Tensor:
        t2 = t1.f.sigmoid_map(t1)
        ctx.save_for_backward(t2)
        return t2
...
class ReLU(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        (t2,) = ctx.saved_values
        return grad_output.f.relu_back_zip(t2, grad_output)
...
class Log(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        (t2,) = ctx.saved_values
        return grad_output.f.log_back_zip(t2, grad_output)
